        ...
        <main class="nodrag">
            <div class="left">
                <div class="handle-container">
                    <Handle id="in-start" type="target" :connectable="1" :position="Position.Left"
                        :is-valid-connection="onConnect" class="handle"
                        style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>1</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="start" placeholder="初始值" :disabled="attrs.data.startHandle"></Input>
                </div>
                <div class="handle-container">
                    <Handle id="in-step" type="target" :connectable="1" :position="Position.Left"
                        :is-valid-connection="onConnect" class="handle"
                        style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>1</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="step" placeholder="步长" :disabled="attrs.data.stepHandle"></Input>
                </div>
                <div class="handle-container">
                    <Handle id="in-end" type="target" :connectable="1" :position="Position.Left"
                        :is-valid-connection="onConnect" class="handle"
                        style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>1</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="end" placeholder="目标值" :disabled="attrs.data.endHandle"></Input>
                </div>
            </div>
            <div class="right">
                <div class="handle-container" v-for="(handle, index) in handles" :key="handle">
                    <Handle :id="`out-^after^-${index}`" type="source" :position="Position.Right"
                ...
                <div class="handle-container">
                    <Handle id="out-index" type="source" :position="Position.Right" :is-valid-connection="onConnect"
                        class="handle" style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>N</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="index" placeholder="索引"></Input>
                </div>
            </div>
        </main>
    </div>
...
const start = toRef(attrs.data, "start");
const step = toRef(attrs.data, "step");
const end = toRef(attrs.data, "end");
const index = toRef(attrs.data, "index");

const handles = toRef(attrs.data, "^after^");
