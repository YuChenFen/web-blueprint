    ...
    <div class="node-container">
        ...
        <main class="nodrag">
            <div class="left">
                ...
                <div class="handle-container">
                    <Handle id="in-width" type="target" :connectable="1" :position="Position.Left"
                        :is-valid-connection="onConnect" class="handle"
                        style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>1</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="width" placeholder="width" :disabled="attrs.data.widthHandle"></Input>
                </div>
                <div class="handle-container">
                    <Handle id="in-height" type="target" :connectable="1" :position="Position.Left"
                        :is-valid-connection="onConnect" class="handle"
                        style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>1</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="height" placeholder="height" :disabled="attrs.data.heightHandle"></Input>
                </div>
            </div>
        </main>
    </div>
...
const width = toRef(attrs.data, "width");
const height = toRef(attrs.data, "height");
const isFill = toRef(attrs.data, "isFill");

const { removeNodes } = useVueFlow();

const node_width = attrs.data.width || 150;
