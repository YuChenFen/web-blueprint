        ...
        <main class="nodrag">
            <div class="left">
                <div class="handle-container">
                    <Handle id="in-number" type="target" :connectable="1" :position="Position.Left"
                        :is-valid-connection="onConnect" class="handle"
                        style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>1</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="number" placeholder="数值" :disabled="attrs.data.value1Handle"></Input>
                </div>
                <div class="handle-container">
                    <Handle id="in-bit" type="target" :connectable="1" :position="Position.Left"
                        :is-valid-connection="onConnect" class="handle"
                        style="background-color: #fff;border-color: #fff;border-radius: 0;">
                        <p>1</p>
                    </Handle>
                    <div class="handle"></div>
                    <Input v-model="bit" placeholder="位数" :disabled="attrs.data.value1Handle"></Input>
                </div>
            </div>
            <div class="right">
                <div class="handle-container">
...
const number = toRef(attrs.data, "number");
const bit = toRef(attrs.data, "bit");

attrs.data.output = computed(() => {
    return `(${number.value} << ${bit.value})`
});
